...
export class OrdersService implements OnModuleInit {
        ...
        try {
            ...
            for (const cartItem of cart.items) {
                ...
                if (variant.stock < cartItem.quantity) {
                    throw new BadRequestException(`Insufficient stock for product ${productData.name} (${variant.size} - ${variant.color}). Only ${variant.stock} left.`);
                }
            ...
            this.logger.log(`Order ${createdOrder._id} created in DB for user ${userId}`);

            await this.cartsService.clearCart(userId, session);
            this.logger.log(`Cart cleared for user ${userId} within transaction.`);

            await session.commitTransaction();
            this.logger.log(`Transaction committed for order ${createdOrder._id}`);
            await this.redisService.del(`orders:user:${userId}`);
            return createdOrder;
        } catch (error) {
            await session.abortTransaction();
            this.logger.error(`Failed to create order for user ${userId}. Transaction aborted.`, error.stack);
            throw error;
        } finally {
            session.endSession();
        }
    ...
    private async _decreaseProductStock(items: OrderItem[]): Promise<void> {
        this.logger.log(`Preparing to call ProductService to decrease stock for ${items.length} order items...`);
