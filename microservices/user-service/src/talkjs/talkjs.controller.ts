// microservices/talkjs-service/src/talkjs.controller.ts
import {
    Controller,
    Post,
    Get,
...
import { TalkjsService, RolesGuard, Role, JwtAuthGuard } from '@app/common-auth';
...
import { GetTalkjsTokenDto } from './dto/get-talkjs-token.dto';
...
export class TalkjsController {
    ...
    async createOrGetConversation(
        ...
        if (currentUser.role === 'customer') {
            ...
            if (!adminId || adminId === 'default_admin_id') {
                throw new BadRequestException('Admin ID is required for a customer to start a conversation.');
            }
            participantIds = [currentUser._id, adminId];
            conversationId = `customer-${currentUser._id}-admin-${adminId}`;
        } else if (currentUser.role === 'admin') {
            if (!targetCustomerId) {
                throw new BadRequestException('Customer ID is required for admin to start a conversation.');
            }
        ...
        this.logger.log(`Creating or getting conversation with ID: ${conversationId} for participants: ${participantIds.join(', ')}`);
        return this.talkjsCommonService.getOrCreateConversation(conversationId, participantIds);
    }
    ...
    async sendMessage(
        ...
        return this.talkjsCommonService.sendMessage(conversationId, senderId, message);
    }
    ...
    async deleteTalkjsConversation(@Param('conversationId') conversationId: string): Promise<void> {
