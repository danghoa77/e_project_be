...
export class TalkjsService {
  ...
  async upsertUser(
    userId: string,
    attributes: TalkjsUserAttributes,
  ): Promise<any> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to upsert TalkJS user '${userId}': ${errorMessage}`,
      );
      throw new InternalServerErrorException(
        `Failed to upsert user to TalkJS: ${errorMessage}`,
      );
    }
  }
  ...
  async getOrCreateConversation(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        `Failed to manage conversation on TalkJS: ${errorMessage}`,
      );
    }
  }
