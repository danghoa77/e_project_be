  ...
  async verifyGoogleIdToken(idToken: string): Promise<any> {
    try {
      const ticket = await this.client.verifyIdToken({
        idToken,
        audience: this.configService.get<string>('GOOGLE_CLIENT_ID'), // Đảm bảo audience khớp với Client ID của bạn
      });
      const payload = ticket.getPayload();
      if (!payload) {
        this.logger.error(
          'Google ID Token verification failed: payload is undefined',
        );
      ...
      this.logger.error(`Failed to verify Google ID Token: ${errorMessage}`);
      throw new InternalServerErrorException(
        'Failed to verify Google ID Token.',
      );
    }
